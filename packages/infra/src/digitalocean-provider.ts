...
export class DigitalOceanProvider implements ComputeProvider {
  ...
  getVolumeDevicePath(botId: string): string {
    return `/dev/disk/by-id/scsi-0DO_Volume_bot-${botId}-data`;
  }

  async createInstance(params: CreateInstanceParams): Promise<ProviderResourceIds> {
    let volumeId: string;

    if (params.existingVolumeId) {
      volumeId = params.existingVolumeId;
      logger.info("Reusing existing volume", { volumeId, botId: params.botId });
    } else {
      // Create volume first
    ...
    try {
      ...
      if (!params.existingVolumeId) {
        // Only cleanup volume if we created it
